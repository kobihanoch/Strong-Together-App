...
import { AnalysisContextExerciseTrackingMaps } from '../../context/types/analysisContextTypes.dto';
...
type CalendarStripCustomProps = {
  ...
  selectedDate: string;
  userExerciseLogs: AnalysisContextExerciseTrackingMaps['byDate'] | null;
};
...
const CalendarStripCustom = ({ onDateSelect, selectedDate, userExerciseLogs }: CalendarStripCustomProps) => {
  ...
  const handleDatePress = (date: moment.Moment) =>
    onDateSelect && onDateSelect(date.clone().tz(timezone).format('YYYY-MM-DD'));
  const selectedDateMoment = useMemo(() => moment.tz(selectedDate, 'YYYY-MM-DD', timezone), [selectedDate]);
  ...
  const flatListRef = useRef<FlatList<Moment> | null>(null);
    ...
    ({ item }: { item: moment.Moment }) => {
      ...
      return (
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Text style={[styles.dayName, isSelected ? styles.selectedText : isToday ? styles.todayText : null]}>
            {item.format('dd')}
          </Text>
          ...
          {splitName ? (
            <Text
              style={{
                ...
                backgroundColor: isSelected ? 'transparent' : 'rgba(234, 240, 246, 0)',
                padding: height * 0.01,
