...
export function splitsCounterToPieData(splitsCounter: Record<string, number> | Map<string, number>) {
  // Normalize entries from Map or plain object
  const entries =
    splitsCounter instanceof Map ? Array.from(splitsCounter.entries()) : Object.entries(splitsCounter ?? {});
