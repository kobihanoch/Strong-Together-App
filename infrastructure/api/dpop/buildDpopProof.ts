...
export default async function buildDpopProof(htm: string, htu: string, at: string | null = null): Promise<string> {
  try {
    ...
    return await new SignJWT(payload).setProtectedHeader(protectedHeader).sign(privateJwkForJose);
  } catch (err) {
