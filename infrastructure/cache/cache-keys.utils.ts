...
export const keyTracking = (userId: string, days: number = 45): string => `CACHE:TRACKING:${userId}:${days}:${CACHE_VERSION}`;
...
export const keyHomeDashboard = (userId: string): string => `CACHE:HOMEDASHBOARDSTATS:${userId}:${CACHE_VERSION}`;
