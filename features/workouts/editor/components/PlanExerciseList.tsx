...
const PlanExerciseList = ({ exercises, exercisesById, expandedExerciseId, horizontalPadding, theme, onToggle, onRemove, onReorder, onSetCount, onRep }: Props) => {
  const { height } = useWindowDimensions();
  const bottomSpace = Math.max(400, Math.min(height * 0.55, 520));
  const renderItem = useCallback(({ item, getIndex, drag }: { item: ExerciseInput; getIndex: () => number | undefined; drag: () => void }) => {
        ...
        <View style={[styles.item, { borderColor: theme.border, backgroundColor: theme.surface }]}>
          ...
          <Pressable style={styles.main} onPress={() => onToggle(item.exerciseId)}>
            ...
            <View style={styles.copy}>
              <Text numberOfLines={1} style={[styles.name, { color: theme.textPrimary }]}>{metadata?.name ?? 'Exercise'}</Text>
              <Text numberOfLines={1} style={[styles.meta, { color: theme.textSecondary }]}>{metadata?.targetMuscle ?? ''}</Text>
            </View>
          ...
          {expanded && <Animated.View entering={FadeIn.duration(110)} style={[styles.editor, { borderTopColor: theme.border }]}>
            <Text style={[styles.label, { color: theme.textSecondary }]}>SETS</Text>
            <View style={styles.optionRow}>
              {[1, 2, 3, 4, 5].map((count) => {
                const selected = count === item.sets.length;
                return <Pressable key={count} onPress={() => onSetCount(item.exerciseId, count)} style={[styles.setOption, { borderColor: theme.border }, selected && { backgroundColor: theme.primary, borderColor: theme.primary }]}>
            ...
            <View style={styles.repsRow}>
              {item.sets.map((reps, setIndex) => <View key={setIndex} style={[styles.repField, { backgroundColor: theme.surfaceMuted, borderColor: theme.border }]}>
                <Text style={[styles.repLabel, { color: theme.textSecondary }]}>S{setIndex + 1}</Text>
                <TextInput value={String(reps)} onChangeText={(value) => onRep(item.exerciseId, setIndex, Number(value.replace(/\D/g, '')) || 1)} keyboardType="number-pad" selectTextOnFocus maxLength={3} style={[styles.repInput, { color: theme.textPrimary }]} />
              </View>)}
            </View>
...
const styles = StyleSheet.create({
  ...
  repField: { minWidth: 84, height: 42, borderRadius: 10, borderWidth: 1, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10 },
