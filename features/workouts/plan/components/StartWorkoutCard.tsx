...
import { HomePageData } from '../../../../hooks/types/useHomePageTypes.dto';
import { RootParamList } from '../../../../navigation/types/appStackTypes';
import { getBodyPartsForSplit } from '../utils/workout-plan.util';
import Badge from '../../../../components/Badge';
import NumberCounter from '../../../../components/NumberCounter';
import PercantageCircle from '../../../../components/PercentageCircle';
...
const StartWorkoutCard = ({ data }: StartWorkoutCardProps) => {
  ...
  const bodyPartsForMostFrqSplit = useMemo(() => {
    if (!hasData) return '';
    const filtered = workoutSplits?.filter((ws) => ws.name === data.mostFrequentSplit?.splitName);
    return getBodyPartsForSplit(filtered?.[0]);
  }, [workoutSplits, data, hasData]);

  const exCountForMostFrqSplit = useMemo(() => {
    if (!hasData || data.mostFrequentSplit?.splitName == null || !workoutForEdit) return 0;
    return workoutForEdit[data.mostFrequentSplit.splitName].length || 0;
  }, [workoutForEdit, data, hasData]);

  const progress = useMemo(() => {
    return hasData ? Math.round((mostFrequentSplit!.times! / totalWorkoutNumber) * 100) : 0;
    ...
    <Skeleton.Group show={isLoading}>
      <View style={styles.bgWrapper}>
        ...
        <Image
          ...
          cachePolicy="disk"
        />
        <LinearGradient colors={['#00000092', '#000000b6']} style={styles.container}>
              ...
              {hasData ? (
                <>
                  ...
                  <View style={{ marginTop: 20 }}>
                    <Skeleton colors={['rgba(136, 136, 136, 1)', 'rgba(201, 201, 201, 1)', 'rgba(136, 136, 136, 1)']}>
                      <Text style={styles.muscleGroupText}>{bodyPartsForMostFrqSplit}</Text>
                    </Skeleton>
                  </View>
                ...
                <View style={{ marginTop: 24 }}>
                  ...
                  <View style={styles.emptyCircle}>
                    <MaterialCommunityIcons name="dumbbell" size={RFValue(22)} color="rgba(255,255,255,0.85)" />
                  </View>
                  <Text style={[styles.muscleGroupText, { marginTop: 20 }]}>No history yet</Text>
                  <Text style={styles.emptySubtext}>Create a plan and finish your first workout</Text>
                </View>
              )}
            </View>
          ...
          <View style={{ width: '100%' }}>
            {isLoading ? (
              ...
              !hasTrainedToday ? (
                <SlideToStart
                  onUnlock={() => {
                    const target = workoutSplits.filter((s) => s.name === mostFrequentSplit!.splitName)[0];
                    if (target) {
                ...
                <Text style={styles.alreadyTrainedText}>Already trained today</Text>
              )
            ) : (
