...
import type { ExercisePerformanceEntry } from '../types/exercise-performance.types';
...
const WorkoutPlanExerciseRow = ({ exercise, index, expanded, performance, theme, onToggle }: Props) => {
  ...
  return <Pressable
    ...
    style={({ pressed }) => [
      common.card,
      styles.card,
      { borderColor: theme.border, opacity: pressed ? 0.84 : 1 },
    ]}
  >
    <View style={styles.summaryRow}>
      ...
      <View style={styles.prescription}>
        ...
        <Text style={[styles.reps, { color: theme.textSecondary }]}>
...
const createStyles = (width: number, height: number) => StyleSheet.create({
  ...
  summaryRow: { minHeight: Math.max(72, Math.min(height * 0.096, 84)), flexDirection: 'row', alignItems: 'center', paddingHorizontal: Math.max(12, width * 0.035), paddingVertical: Math.max(10, height * 0.014) },
  ...
  reps: { marginTop: Math.max(2, height * 0.004), fontFamily: fontFamilies.regular, fontSize: fontSizes.label, textAlign: 'right', flexWrap: 'wrap' },
  ...
  performanceSet: { minWidth: Math.max(84, width * 0.235), flexGrow: 1, borderRadius: Math.max(9, width * 0.028), paddingHorizontal: Math.max(8, width * 0.025), paddingVertical: Math.max(7, height * 0.011) },
