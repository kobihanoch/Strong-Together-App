...
const DeltaPill = memo(function DeltaPill({ delta }: { delta: number }) {
  ...
  return (
    <View style={[pillStyles.pill, { backgroundColor: bg, borderColor: border }]}>
      ...
      <Text style={[pillStyles.text, { color: fg }]} numberOfLines={1} ellipsizeMode="clip">
        {label}
      </Text>
    </View>
  );
...
function StatsTable({
  ...
  return (
    <View style={styles.tableCard}>
      ...
      <FlatList
        data={rows}
        keyExtractor={(it, i) => String(it?.setNo ?? i)}
