...
import { UseCreateWorkoutLogicReturn } from '../../hooks/types/useCreateWorkoutTypes.dto';
...
jest.mock('../../hooks/logic/useCreateWorkoutLogic', () => ({
  __esModule: true,
  default: () => mockLogic(),
}));

jest.mock('../../components/CreateWorkoutComponents/TopSection', () => {
  ...
  return function MockTopSection(props: any) {
    return ReactLocal.createElement(
      ...
      ReactLocal.createElement(
        TouchableOpacity,
        { onPress: props.addSplit },
        ReactLocal.createElement(Text, null, 'top-add'),
      ),
      ReactLocal.createElement(
        TouchableOpacity,
      ...
      ReactLocal.createElement(
        TouchableOpacity,
        { onPress: props.saveWorkout },
        ReactLocal.createElement(Text, null, 'top-save'),
      ),
    );
  };
});

jest.mock('../../components/CreateWorkoutComponents/SelectedExercisesList', () => {
  const ReactLocal = require('react');
...
jest.mock('../../components/CreateWorkoutComponents/ExercisePickerModal', () => {
  const ReactLocal = require('react');
  const { Text, View } = require('react-native');
  return ReactLocal.forwardRef((props: any, ref: any) => {
    ...
    return ReactLocal.createElement(
      ...
      ReactLocal.createElement(
        Text,
        null,
        `picker:${props.selectedSplit}:${props.muscles.join('|')}:${props.allExercises.length}`,
      ),
    );
  });
});
