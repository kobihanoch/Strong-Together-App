...
import { ExerciseAnalysisOverview } from '../StartWorkout';
...
const AnalyzeExerciseSheet = ({
  ...
  const hydrateVideoDetails = useCallback(async (uri: string, fallback?: Partial<VideoSelectionState>) => {
    try {
      ...
      setSelectedVideo((prev) => ({
        ...
        durationMs: typeof meta?.duration === 'number' ? meta.duration * 1000 : (fallback?.durationMs ?? prev?.durationMs ?? null),
      }));
    } catch {
      setSelectedVideo((prev) => ({
        uri,
  ...
  }, [analysisResults, isCompressing, onCacheAnalysis, phase, selectedExercise?.exercise, selectedExercise?.id, syncAnalysisOverview]);
  ...
  const repsNeedingTorsoFix = useMemo(() => completedResults.filter((rep) => rep.backLean.excessive).length, [completedResults]);
  ...
  return (
    <ScrollView ref={scrollViewRef} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <AnalyzePrimaryAction hasSelectedVideo={!!selectedVideo} isBusy={isBusy} isPickingVideo={isPickingVideo} onPress={handlePickVideo} />
      <AnalyzeClipStatusCard
