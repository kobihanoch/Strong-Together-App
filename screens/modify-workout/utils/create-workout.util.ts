...
export const addExerciseLogic = (prev: SelectedExercises, splitName: string, exercise: ExerciseCandidate): SelectedExercises => {
...
export const removeExerciseLogic = (prev: SelectedExercises, splitName: SplitName, exercise: ExerciseCandidate): SelectedExercises => {
  const splitExercises = prev[splitName] ?? [];
  const isExExists = splitExercises.some((ex) => ex.id === exercise.id);
  if (isExExists) {
...
export const updateSetsLogic = (
  ...
  const isExExists = splitExercises.some((ex) => ex.id === exercise.id);
  if (isExExists) {
    const updatedSplitExercises = splitExercises.map((ex) => (ex.id === exercise.id ? { ...ex, sets: updatedSetsArr } : ex));
    return {
      ...prev,
...
export const addSplitLogic = (prev: SelectedExercises): { next: SelectedExercises; lastAdded: SplitName | null; didAdd: boolean } => {
...
export const onDragEndLogic = (prev: SelectedExercises, splitName: SplitName, data: SelectedExercise[]): SelectedExercises => {
  // return new state (immutable!)
  return { ...prev, [splitName]: reindexExercises(data) };
};

export const removeSplitLogic = (prev: SelectedExercises, splitName: SplitName): { next: SelectedExercises; nextSelected: SplitName } => {
  const copy = { ...prev };
