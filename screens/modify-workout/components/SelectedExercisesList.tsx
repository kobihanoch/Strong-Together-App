...
import NumericInputWithRules from '../../start-workout/components/NumericInputWithRules';
...
const ExerciseDraggableItem = React.memo(({ item, drag, removeExercise, updateSets, selectedSplit }: ExerciseDraggableItemProps) => {
  ...
  const handleChangeAt = (idx: number) => (val: number) => {
  ...
  const handleRemoveExercise = (ex: SelectedExercise) => {
    removeExercise(ex);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    renderSetsChoose.push(
      <TouchableOpacity
        style={[styles.setContainer, isSetChose ? { backgroundColor: colors.primary } : {}]}
        key={i}
        onPress={() => {
          const n = i + 1;
          setSetsChose(n);
      ...
      >
        <Text style={[styles.setText, , isSetChose ? { color: 'white' } : {}]}>{i + 1}</Text>
      </TouchableOpacity>,
    );
  }
    ...
    <ScaleDecorator activeScale={0.98}>
      <Column style={styles.exerciseItemContainer}>
        <Row style={{ gap: 10, alignItems: 'center' }}>
          <Pressable onLongPress={drag} delayLongPress={120} style={styles.heandleDrag}>
            <MaterialCommunityIcons name={'drag'} size={RFValue(20)} color={'black'} style={{ opacity: 0.4 }}></MaterialCommunityIcons>
          </Pressable>
          <Column style={{ gap: 2 }}>
          ...
          <TouchableOpacity style={{ marginLeft: 'auto' }} onPress={() => handleRemoveExercise(item)}>
            <MaterialCommunityIcons name={'close'} size={RFValue(15)} color={'black'} style={{ opacity: 0.4 }}></MaterialCommunityIcons>
          </TouchableOpacity>
        </Row>
        <Column style={styles.setsSectionContainer}>
          <Row style={{ alignItems: 'flex-start' }}>
            ...
            <Text style={styles.setsChoseText}>{setsChose + ' ' + (setsChose === 1 ? 'set' : 'sets')}</Text>
          </Row>
          <Text style={styles.setsHeader}>Reps (Tap to edit)</Text>
          <Row style={{ gap: 5, justifyContent: 'flex-start', flexWrap: 'wrap' }}>
            {setsArr.map((set, i) => {
              return (
                <Column style={styles.setInputContainer} key={i}>
                  <Row style={{ gap: 10 }}>
                    <Text style={{ fontSize: RFValue(10), opacity: 0.4 }}>S{i + 1}</Text>
...
ExerciseDraggableItem.displayName = 'ExerciseDraggableItem';
...
const SelectedExercisesList = ({ exForSplit, controls, selectedSplit }: SelectedExercisesList) => {
  ...
  const renderItem = useCallback(
    ({ item, drag }: { item: SelectedExercise; drag: () => void }) => {
      return (
  ...
  if (exForSplit.length === 0) {
    return (
      <View style={{ flex: 7, justifyContent: 'center' }}>
        <Text style={{ opacity: 0.6, textAlign: 'center', marginTop: 24 }}>No selected exercises yet.</Text>
      </View>
    );
  }
