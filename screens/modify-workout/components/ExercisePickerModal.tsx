...
import { CreateWorkoutControls, ExerciseCandidate, SelectedExercise } from '../types/use-create-workout.types';
import { ExercisesByMuscle } from '../../../features/workouts/plan/types/workout-plan.types';
...
const ExercisePickerModal = forwardRef<SlidingBottomModalRef, ExercisePicketModalProps>(function ExercisePickerModal(
  { selectedSplit, controls, availableExercises, allExercises, muscles, exForSplit }: ExercisePicketModalProps,
  ref,
) {
  ...
  const addExerciseRef = useRef<(exercise: ExerciseCandidate) => void | null>(null);
  useEffect(() => {
    addExerciseRef.current = addExercise;
  }, [addExercise]);

  const handleExPress = useCallback((ex: ExerciseCandidate) => {
  ...
  const exToShow = selectedMuscle === 'All' ? allExercises : availableExercises[selectedMuscle];

  const [query, setQuery] = useState('');
  const filteredExToShow = useMemo(() => exToShow.filter((ex) => ex.name!.toLowerCase().includes(query.toLowerCase())), [exToShow, query]);
  const isLibraryEmpty = allExercises.length === 0;
  const hasNoSearchResults = !isLibraryEmpty && filteredExToShow.length === 0;

  const renderItem = useCallback(
    ({ item }: { item: Exercise }) => <Exercise item={item} selectedMuscle={selectedMuscle} handleExPress={handleExPress} />,
  ...
  return (
    <SlidingBottomModal ref={ref} snapPoints={['60%', '60%', '80%']} flatListUsage={false} title={''}>
      <Pressable onPress={Keyboard.dismiss} accessible={false}>
        <Column style={{ paddingHorizontal: 15, gap: 20 }}>
          ...
          <Column style={{ marginTop: 15, gap: 5 }}>
            ...
            <Text style={styles.semiHeader}>Choose from our exercise library</Text>
          </Column>

          {/* Search Bar */}
          <Row style={styles.searchBarContainer}>
            <MaterialCommunityIcons name={'magnify'} color={'#aaaaaaff'} size={RFValue(17)} />
            <TextInput
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ overflow: 'visible' }}>
            {muscles?.map((muscle) => {
              ...
              return (
                <TouchableOpacity
                  style={[styles.muscleTab, isMuscleSelected ? { backgroundColor: colors.primary } : {}]}
                  key={muscle}
                  onPress={() => setSelectedMuscle(muscle)}
                >
                  <Text style={[styles.muscleTabText, isMuscleSelected ? { color: 'white', fontFamily: 'Inter_600SemiBold' } : {}]}>
                    {muscle}
                  </Text>
                </TouchableOpacity>
              );
