...
export default function VerifyCard({ username, password, initialEmail }: VerifyCardProps) {
  ...
  const [passwordForChangeEmail, setPasswordForChangeEmail] = useState<string>('');
  ...
  const verifyCooldownUntilRef = useRef<number | null>(null);
  const sendEmailLockRef = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    const sub = AppState.addEventListener('change', async (state) => {
      ...
      const { isVerified } = await checkUserVerify(username);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      const remaining = Math.max(0, Math.ceil((verifyCooldownUntilRef.current! - Date.now()) / 1000));
  ...
  const handleLoginPress = async () => {
    if (!username || !password) {
      showErrorAlert('Missing credentials', 'Please go back and log in from the main form');
      return;
    }
    const { isVerified } = await checkUserVerify(username);
    if (!isVerified) {
  ...
  const handleSubmitChangeEmail = async () => {
    ...
    if (changeEmailLockRef.current >= 3) {
      showErrorAlert('Too many requests', 'You requested too many verification emails.');
      return;
    }
  ...
  const handleResendVerify = async () => {
    ...
    if (!targetEmail) {
      showErrorAlert('Missing email', 'No email is available to send verification to');
      return;
    }
  ...
  return (
    <View style={styles.verifyCard}>
      <View style={styles.verifyIconCircle}>
        <MaterialCommunityIcons name="email-check-outline" size={RFValue(28)} color="#2ecc71" />
      </View>
      ...
      {showChange && (
        <View style={styles.changeForm}>
          ...
          <TouchableOpacity
            style={[styles.btnPrimary, { marginTop: 12, opacity: changing ? 0.7 : 1 }]}
            onPress={handleSubmitChangeEmail}
            disabled={changing}
          >
            <View style={styles.buttonContent}>
              {changing ? <ActivityIndicator /> : <Text style={styles.btnPrimaryText}>Save & resend</Text>}
            </View>
          </TouchableOpacity>
        </View>
      )}
      <Text style={styles.verifyHint}>It can take up to a minute. Also check your spam folder.</Text>
      <TouchableOpacity onPress={handleResendVerify} disabled={isResendingVerify || verifyCooldown > 0}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          >
            {verifyCooldown > 0 ? `Resend verification in ${verifyCooldown}s` : "I didn't receive an email, Send me again"}
          </Text>
        </View>
      </TouchableOpacity>
    </View>
