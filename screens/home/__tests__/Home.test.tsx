...
describe('Home', () => {
  ...
  it('renders the create-workout state when no plan exists', () => {
    mockLogic = {
      data: { ...data, state: { hasWorkout: false, hasTracking: false } },
      actions: mockActions,
      loadingStates: { isPending: false, isFetching: false },
    };
  ...
  it('renders the no-tracking state for a plan without history', () => {
    mockLogic = {
      data: { ...data, state: { hasWorkout: true, hasTracking: false } },
      actions: mockActions,
      loadingStates: { isPending: false, isFetching: false },
    };
    const { getByText, queryByTestId } = render(<Home />);
