...
import useStartWorkoutPageLogic from '../useStartWorkoutPageLogic';
...
const ReadyGate = ({ children }: { children: React.ReactNode }) => {
  const { user } = useAuth();
  const { workout, loading: workoutLoading } = useWorkoutContext();
  const { loading: analysisLoading } = useAnalysisContext();
...
describe('useStartWorkoutPageLogic integration', () => {
  beforeEach(() => {
    jest.clearAllMocks();
