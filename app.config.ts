...
const buildProfile = process.env.APP_PROFILE || process.env.EAS_BUILD_PROFILE || 'development';
...
const config = ({ config }: ConfigContext): ExpoConfig => {
  ...
  if (isProd && iosBundle.endsWith('.dev')) {
    throw new Error('Prod build detected but bundleIdentifier resolved to .dev');
  }
